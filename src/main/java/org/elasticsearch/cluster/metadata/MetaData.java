...
public class MetaData implements Iterable<IndexMetaData> {
    ...
    MetaData(long version, Settings transientSettings, Settings persistentSettings, Map<String, IndexMetaData> indices, Map<String, IndexTemplateMetaData> templates, Map<String, Custom> customs) {
        ...
        /*for (int i = 0; i < tmpAliases.allocated.length; i++) {
            if (tmpAliases.allocated[i]) {
                tmpAliases.values[i] = XMaps.makeReadOnly((ObjectObjectOpenHashMap) tmpAliases.values[i]);
            }
        }*/
        ...
        /*for (int i = 0; i < aliasAndIndexToIndexMap.allocated.length; i++) {
            if (aliasAndIndexToIndexMap.allocated[i]) {
                Object[] bla = aliasAndIndexToIndexMap.values;
                ((StringArray) bla[i]).trim();
            }
        }*/
