        ...
        for (IndexMetaData indexMetaData : indices.values()) {
            ...
            for (AliasMetaData aliasMd : indexMetaData.aliases().values()) {
                ...
                if (indexAliasMap == null) {
                    indexAliasMap = new ExtTHashMap<String, AliasMetaData>();
                    aliases.put(aliasMd.alias(), indexAliasMap);
                }
                // set routing alias values into AliasMetaData
                if (aliasMd.searchRouting() != null) {
                    aliasMd.searchRoutingValues(Collections.unmodifiableSet(Strings.splitStringByCommaToSet(aliasMd.searchRouting())));
                }
