...
public class MultiPercolateRequest extends ActionRequest<MultiPercolateRequest> {
            ...
            if (nextMarker - from > 0) {
                ...
                try {
                    ...
                    if (token != null) {
                        ...
                        String percolateOperation = parser.currentName();
                        if ("percolate".equals(percolateOperation)) {
                            parsePercolateAction(parser, percolateRequest);
                        } else if ("count_percolate".equals(percolateOperation) || "countPercolate".equals(percolateOperation)) {
                            percolateRequest.onlyCount(true);
                            parsePercolateAction(parser, percolateRequest);
                        } else if ("percolate_existing_doc".equals(percolateOperation) || "percolateExistingDoc".equals(percolateOperation)) {
                            parsePercolateExistingAction(parser, percolateRequest);
                        } else if ("count_percolate_existing_doc".equals(percolateOperation) || "countPercolateExistingDoc".equals(percolateOperation)) {
                            percolateRequest.onlyCount(true);
                            parsePercolateExistingAction(parser, percolateRequest);
                        } else {
                            throw new ElasticSearchParseException(percolateOperation + " isn't a supported percolate operation");
                        }
                    }
                } finally {
                    parser.close();
    ...
    private void parsePercolateExistingAction(XContentParser parser, PercolateRequest percolateRequest) throws IOException {
        ...
        if (percolateRequest.indices() == null && percolateRequest.getRequest().index() != null) {
