...
/**
 */
public final class CompoundTermsEnum extends TermsEnum {

    public static MultiTermsEnum getCompoundTermsEnum(IndexReader indexReader, String... fields) throws IOException {
    ...
    public static MultiTermsEnum getCompoundTermsEnum(AtomicReader atomicReader, String... fields) throws IOException {
        List<TermsEnum> fieldEnums = new ArrayList<TermsEnum>();
        for (int i = 0; i < fields.length; i++) {
        ...
        return new CompoundTermsEnum(fieldEnums.toArray(new TermsEnum[fieldEnums.size()]));*/
    }
    ...
    private CompoundTermsEnum(TermsEnum[] enums) {
        this.comparator = enums[0].getComparator();
        states = new ArrayList<TermsEnumState>(enums.length);
        for (TermsEnum tEnum : enums) {
            states.add(new TermsEnumState(tEnum));
        }

        stateSlots = new IntArrayList(enums.length);
    }
    ...
    public BytesRef next() throws IOException {
        ...
        BytesRef lowestTerm = states.get(0).current;
        stateSlots.add(0);
        for (int i = 1; i < states.size(); i++) {
            TermsEnumState state = states.get(i);
            int cmp = lowestTerm.compareTo(state.current);
            if (cmp > 0) {
                lowestTerm = state.current;
                stateSlots.clear();
                stateSlots.add(i);
            } else if (cmp == 0) {
                stateSlots.add(i);
    ...
    public SeekStatus seekCeil(BytesRef text) throws IOException {
        ...
        int removed = 0;
        for (TermsEnumState state : states) {
            SeekStatus status = state.seekCeil(text);
...
//                states.remove();
    ...
    class TermsEnumState {
        ...
        BytesRef next() throws IOException {
            return current = termsEnum.next();
        }

        SeekStatus seekCeil(BytesRef text) throws IOException {
            SeekStatus seekStatus = termsEnum.seekCeil(text);
            current = termsEnum.term();
            return seekStatus;
        }
    }
